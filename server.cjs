...
// Ex.: /data/dados-atualizados.json
...
app.post('/api/update-data', async (_req, res) => {
  try {
    console.log('🔄 Atualização manual solicitada (POST)...');
...
// Atalho GET para atualizar e redirecionar ao JSON (útil no browser)
app.get('/update', async (_req, res) => {
  try {
    console.log('🔄 Atualização manual solicitada (GET /update)...');
    await fetchDataFromSharePoint();
    res.redirect('/data/dados-atualizados.json');
  } catch (e) {
    console.error('❌ Falha no /update:', e);
    res.status(500).send('Falha na atualização: ' + (e?.message || e));
  }
});
